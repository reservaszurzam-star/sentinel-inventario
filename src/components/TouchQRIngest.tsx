...
// Calibrated Textile Color Palette Mapping for Apparel
...
const TEXTILE_COLOR_MAP: Record<string, ColorStyle> = {
  ...
  AZUL: { hex: '#182b49', textLight: false }, // Azul Marino profundo real
  NAVY: { hex: '#182b49', textLight: false },
  DENIM: { hex: '#375375', textLight: false }, // Azul Jean / Índigo lavado
  CELESTE: { hex: '#8cb9de', textLight: true },
  BOTELLA: { hex: '#113a24', textLight: false }, // Verde bosque oscuro profundo
  VERDE: { hex: '#1e6837', textLight: false },
  MENTA: { hex: '#9ad0b4', textLight: true },
  PACAY: { hex: '#779834', textLight: false }, // Verde oliva / pacay suave
  CAMOTE: { hex: '#b75232', textLight: false }, // Terracota / teja real textil
  CEMENTO: { hex: '#878d95', textLight: true }, // Gris cemento neutro
  PLOMO: { hex: '#475569', textLight: false },
  TOPO: { hex: '#7a7167', textLight: false },
  MARRON: { hex: '#482a1d', textLight: false }, // Marrón chocolate
  CAFE: { hex: '#482a1d', textLight: false },
  CAMEL: { hex: '#af7b44', textLight: false },
  VINO: { hex: '#5c1726', textLight: false }, // Vino tinto oscuro
  GUINDA: { hex: '#5c1726', textLight: false },
  ROJO: { hex: '#b91c1c', textLight: false },
  'P.ROSA': { hex: '#cb8e95', textLight: true }, // Palo Rosa empolvado real
...
function getColorStyle(colorName?: string): ColorStyle {
  if (!colorName) return { hex: '#878d95', textLight: true };
  const clean = colorName.trim().toUpperCase();
  for (const key of Object.keys(TEXTILE_COLOR_MAP)) {
    if (clean.includes(key)) return TEXTILE_COLOR_MAP[key];
  }
  return { hex: '#878d95', textLight: true };
}
